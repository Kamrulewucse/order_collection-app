...
@section('content')
    ...
    <!-- form start -->
    <form enctype="multipart/form-data" action="{{ route('requisition-order.make_order',['requisitionOrder'=>$requisitionOrder->id]) }}" class="form-horizontal" method="post">
        @csrf
        <div class="row">
        <!-- left column -->
        <div class="col-md-9">
            ...
            <div class="card card-default">
                            ...
                            <div class="form-group {{ $errors->has('client') ? 'has-error' :'' }}">
                                ...
                                <select class="form-control select2" id="client" name="client" required>
                                    ...
                                    @foreach($clients as $client)
                                        <option {{ old('client',$requisitionOrder->client_id) == $client->id ? 'selected' : ''  }} value="{{ $client->id }}" data-balance="{{ $client->balance }}">{{ $client->name }}</option>
                                    @endforeach
                                </select>
                                @error('client')
                ...
                <div class="card-header">
                   <div class="row">
                       ...
                       <div class="col-md-2">
                           <div class="form-group">
                               <label for="add_quantity">Requisition Qty <span class="text-danger">*</span></label>
                ...
                <!-- /.card-header -->
                                ...
                                <table class="table table-bordered">
                                   <thead>
                                       <tr>
                                           ...
                                           <th class="text-center" width="12%">Requisition Qty <span class="text-danger">*</span></th>
                                    ...
                                    <tbody id="product-container">
                                        @if (old('product_id') != null && sizeof(old('product_id')) > 0)
                                            ...
                                            @endforeach
                                        @else
                                            @foreach($requisitionOrder->requisitionOrderItems as $key => $item)
                                                <tr class="product-item">
                                                    <td class="text-center">
                                                        <span class="product_sl">{{ ++$key }}</span>
                                                    </td>
                                                    <td class="text-left">
                                                        ...
                                                        <input type="hidden" name="product_name_val[]" value="{{ $item->product->name }}" class="product_name_val">
                                                        <input type="hidden" name="product_id[]" value="{{ $item->product_id }}" class="product_id">
                                                    </td>
                                                    <td class="text-right">
                                                        <div class="form-group mb-0">
                                                            <input type="number" step="any" value="0" class="form-control text-right damage_return_product_qty" name="damage_return_product_qty[]">
                                                        </div>
                                                    </td>
                                                    <td class="text-right">
                                                        <div class="form-group mb-0">
                                                            <input type="number" step="any" value="{{ $item->sr_sale_quantity }}" class="form-control text-right product_qty" name="product_qty[]">
                                                        </div>
                                                    </td>
                                                    <td class="text-right">
                                                        <div class="form-group mb-0">
                                                            <input type="number" step="any" value="{{ $item->purchase_unit_price }}" class="form-control text-right purchase_unit_price" name="purchase_unit_price[]">
                                                        </div>
                                                    </td>
                                                    <td class="text-right">
                                                        <div class="form-group mb-0">
                                                            <input type="number" step="any" value="{{ $item->selling_unit_price }}" class="form-control text-right selling_unit_price" name="selling_unit_price[]">
                                                        </div>
                                                    </td>
                                                    <td class="text-right total-purchase-cost"></td>
                                                    <td class="text-center"><button type="button" class="btn btn-danger bg-gradient-danger btn-sm btn-remove"><i class="fa fa-trash-alt"></i></button></td>
                    ...
                    <div class="card-footer">
                        ...
                        <a href="{{ route('sales-order.index') }}" class="btn btn-danger bg-gradient-danger btn-sm float-right">Cancel</a>
                    </div>
                    <!-- /.card-footer -->
            </div>
            <!-- /.card -->
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-12">
                            <div class="form-group {{ $errors->has('balance') ? 'has-error' :'' }}">
                                ...
                                <input type="text" value="{{ old('balance',0) }}" name="balance" id="balance" class="form-control" placeholder="Enter Balance" readonly>
                                @error('balance')
                                <span class="help-block">{{ $message }}</span>
                                @enderror
                            </div>
