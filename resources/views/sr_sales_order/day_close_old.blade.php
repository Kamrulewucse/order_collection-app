            ...
            <div class="card card-default">
                <div class="card-header">
                    @if ($distributionOrder->hold_release != 1)
                        @if ($distributionOrder->close_status == 1)
                            <a href="{{ route('distribution.day_close', ['distributionOrder' => $distributionOrder->id, 'close_status' => 1, 'type' => 1]) }}"
                                class="btn btn-primary bg-gradient-primary btn-sm">Edit <i class="fa fa-edit"></i></a>
                        @else
                            <a href="{{ route('distribution.day_close', ['distributionOrder' => $distributionOrder->id, 'type' => 1]) }}"
                                class="btn btn-primary bg-gradient-primary btn-sm">Edit Close <i class="fa fa-edit"></i></a>
                        @endif
                        <a data-id="{{ $distributionOrder->id }}" role="button"
                            class="btn btn-warning bg-gradient-warning btn-sm day-close"><i class="fa fa-info-circle"></i>
                            Hold Release</a>
                    @endif
                ...
                <div class="card-body">
                    <div class="table-responsive" id="printAreaChallan">
                        <form
                            action="{{ route('distribution.day_close', ['distributionOrder' => $distributionOrder->id, 'type' => request('type')]) }}"
                            method="post">
                            @csrf
                            ...
                            <div class="row pt-3 pb-3">
                                <div class="col-4">
                                    <h5 class="text-center m-0">SR: {{ $distributionOrder->sr->name ?? '' }}</h5>
                                </div>
                                <div class="col-4">
                                    <h5 class="text-center m-0">ORDER NO. {{ $distributionOrder->order_no }}</h5>
                                    <h6 class="text-center m-0">Client: {{ $distributionOrder->sr->name }}</h6>
                                </div>
                                <div class="col-4">
                                    <h5 class="text-center m-0">DATE:
                                        {{ \Carbon\Carbon::parse($distributionOrder->date)->format('d-m-Y') }}</h5>
                                </div>
                            </div>
                                ...
                                <tbody>
                                    ...
                                    @foreach ($distributionOrder->distributionOrderItems as $item)
                                        <tr class="product-item">
                                            ...
                                            <td class="text-right">
                                                ...
                                                @else
                                                    <input type="hidden" name="return_quantity[]" step="any"
                                                        class="form-control text-right text-bold return_quantity"
                                                        value="{{ $item->return_quantity }}">
                                                    {{ number_format($item->return_quantity) }}
                                                @endif

                                                <input type="hidden" name="distribute_quantity[]"
                                                    class="distribute_quantity" value="{{ $item->distribute_quantity }}">
                                            ...
                                            <td class="text-right total-sale">
                                                {{ number_format($item->sale_quantity * $item->selling_unit_price, 2) }}
                                            </td>
                                        </tr>
                                        @php
                                    ...
                                    @if ($distributionOrder->close_status == 0)
                                        <tr>
                                            ...
                                            <th class="text-right">
                                                {{ number_format($distributionOrder->distributionOrderItems->sum('distribute_quantity')) }}
                                            </th>
                                            <th class="text-right" id="total_return_quantity">
                                                {{ number_format($distributionOrder->distributionOrderItems->sum('return_quantity')) }}
                                            </th>
                                            <th class="text-right" id="total_selling_quantity">
                                                {{ number_format($distributionOrder->distributionOrderItems->sum('sale_quantity')) }}
                                            </th>
                                            <th class="text-right" id="total_sale_price">
                                                {{ number_format($totalPurchasePrice, 2) }}</th>
                                        </tr>
                                    @endif
                                </tbody>
                                <tfoot>
                                    @if ($distributionOrder->close_status == 0)
                                        <tr class="extra-remove">
                                            <th class="text-right" colspan="6"><button
                                                    class="btn btn-primary bg-gradient-primary">Adjust Update</button></th>
                                            <th></th>
                                        </tr>
                                    @elseif($distributionOrder->saleOrders->count() === 0)
                                        <tr>
                                            <th class="text-right" colspan="6">Total</th>
                                            <th class="text-right"> {{ number_format($distributionOrder->total, 2) }}</th>
                                        </tr>
                                        <tr>
                        ...
                        <form
                            action="{{ route('distribution.customer_sale_entry', ['distributionOrder' => $distributionOrder->id, 'type' => request('type')]) }}"
                            method="post">
                            @csrf
                            ...
                            <table id="table" class="table table-bordered mt-5">
                                                ...
                                                <div class="row">
                                                    ...
                                                    <div class="col-md-4">
                                                        <div class="form-group">
                                                            <label for="select_customer">Customer <span
                                                                    class="text-danger">*</span></label>
                                                            <select class="form-control select2" id="select_customer"
                                                                name="select_customer">
                                                                <option value="">Search Name of Customer</option>
                                                                @foreach ($customers as $customer)
                                                                    <option {{ old('select_customer') }}
                                                    ...
                                                    <div class="col-md-2">
                                                        <div class="form-group">
                                                            <label for="add_total">Total Amount<span
                                                                    class="text-danger">*</span></label>
                                                            <input type="number" step="any" class="form-control"
                                                                id="add_total_amount" placeholder="Total Amount">
                                                        </div>
                                                    </div>
                                                    <div class="col-md-2">
                                                        <div class="form-group">
                                                            <label for="add_paid">Paid Amount <span
                                                                    class="text-danger">*</span></label>
                                                            <input type="number" step="any" class="form-control"
                                                                id="add_paid_amount" placeholder="Paid Amount">
                                                        </div>
                                                    </div>
                                                    <div class="col-md-1">
                                                        <div class="form-group">
                                ...
                                @if ($distributionOrder->close_status == 0)
                                    <tfoot>
                                        <tr class="extra-remove">
                                            ...
                                            <th colspan="2" class="text-left">
                                                <div
                                                    ...
                                                    <input type="text"
                                                        value="{{ $paymentMode->name }}- {{ $paymentMode->code }}"
                                                        class="form-control" readonly>
                                                    <input type="hidden" name="payment_mode"
                                                        value="{{ $paymentMode->id }}|{{ $paymentMode->payment_mode }}"
                                                        class="form-control">
                                                    @error('payment_mode')
                                        ...
                                        <tr class="extra-remove">
                                            <th class="text-right" colspan="5"><button
                                                    class="btn btn-primary bg-gradient-primary">Paid Update</button></th>
                                        </tr>
                                    </tfoot>
                                @endif
                            </table>
        ...
        <tr class="customer-item">
            ...
            <td class="text-right">
                <div class="form-group mb-0">
                    <input type="number" step="any" class="form-control text-right total_amount"
                        name="total_amount[]">
                </div>
            </td>
            <td class="text-right">
                <div class="form-group mb-0">
                    <input type="number" step="any" class="form-control text-right paid_amount"
                        name="paid_amount[]">
                </div>
            </td>
            <td class="text-right">
                <div class="form-group mb-0">
                    <input type="number" readonly step="any" class="form-control text-right due_amount"
                        name="due_amount[]">
                </div>
            </td>
    ...
    <script>
            ...
            $('body').on('click', '.day-close', function() {
                ...
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        ...
                        $.ajax({
                            method: "POST",
                            url: "{{ route('distribution.hold_release_post', ['distributionOrder' => 'REPLACE_WITH_ID_HERE']) }}"
            ...
            $('body').on('click', '#add_new_btn', function(e) {
                ...
                if (remaining_amount < addTotalAmount) {
                    Swal.fire({
                        icon: 'error',
                        title: 'Oops...',
                        text: 'Remaining amount (' + remaining_amount + ') exceeds total amount (' +
                            addTotalAmount + ')',
                    });
                ...
                if (selectCustomer != '' && addTotalAmount != '') {
                    ...
                    item.closest('tr').find('.customer_name').text($("#select_customer option:selected")
                        .text());
                    item.closest('tr').find('.customer_name_val').val($("#select_customer option:selected")
                        .text());
                    item.closest('tr').find('.customer_id').val($("#select_customer option:selected")
                .val());
        ...
        function calculate() {
            ...
            $('.product-item').each(function(i, obj) {
                ...
                distribute_quantity = (isNaN(distribute_quantity) || distribute_quantity < 0) ? 0 :
                    distribute_quantity;
