@extends('layouts.app')
@section('title','Hatchery Manager Create')
@section('content')
    <div class="row">
        <!-- left column -->
        <div class="col-md-12">
            ...
            <div class="card card-default">
                ...
                <!-- /.card-header -->
                <!-- form start -->
                <form enctype="multipart/form-data" action="{{ route('hatchery-manager.store') }}" class="form-horizontal" method="post">
                    @csrf
                    <div class="card-body">
                            ...
                            <div class="col-sm-10">
                                <select name="hatchery" id="hatchery" class="form-control select2">
                                    ...
                                    @foreach($hatcheries as $hatchery)
                                        <option {{ old('hatchery') == $hatchery->id ? 'selected' : '' }} value="{{ $hatchery->id }}">{{ $hatchery->name }}</option>
                                    @endforeach
                                </select>
                                @error('hatchery')
                                <span class="help-block">{{ $message }}</span>
                        ...
                        <div class="form-group row {{ $errors->has('designation') ? 'has-error' :'' }}">
                            ...
                            <div class="col-sm-10">
                                <input type="text" value="{{ old('designation') }}" name="designation" class="form-control" id="designation" placeholder="Enter designation">
                                @error('designation')
                                <span class="help-block">{{ $message }}</span>
                                @enderror
                            </div>
