        ...
        <div class="col-md-12">
            ...
            <div class="card card-default">
                <div class="card-header">
                    <h3 class="card-title">Hatchery Information</h3>
                </div>
                <!-- /.card-header -->
                <!-- form start -->
                <form enctype="multipart/form-data" action="{{ route('hatchery.update',['hatchery'=>$hatchery->id]) }}" class="form-horizontal" method="post">
                    @csrf
                    @method('PUT')
                    <div class="card-body">
                        <div class="form-group row {{ $errors->has('name') ? 'has-error' :'' }}">
                            <label for="name" class="col-sm-2 col-form-label">Hatchery Name <span class="text-danger">*</span></label>
                            <div class="col-sm-10">
                                <input type="text" value="{{ old('name',$hatchery->name) }}" name="name" class="form-control" id="name" placeholder="Enter Name">
                                @error('name')
                        ...
                        <div class="form-group row {{ $errors->has('mobile_no') ? 'has-error' :'' }}">
                            ...
                            <div class="col-sm-10">
                                <input type="text" value="{{ old('mobile_no',$hatchery->mobile_no) }}" name="mobile_no" class="form-control" id="mobile_no" placeholder="Enter Mobile No.">
                                @error('mobile_no')
                                <span class="help-block">{{ $message }}</span>
                                @enderror
                            </div>
                        ...
                        <div class="form-group row {{ $errors->has('district') ? 'has-error' :'' }}">
                            ...
                            <div class="col-sm-10">
                                <select name="district" id="district" class="form-control select2">
                                    ...
                                    <option {{ old('district',$hatchery->district_id) == $district->id ? 'selected' : '' }} value="{{ $district->id }}">{{ $district->name_eng }}</option>
                                    @endforeach
                                </select>
                                @error('district')
                                <span class="help-block">{{ $message }}</span>
                        ...
                        <div class="form-group row {{ $errors->has('latitude') ? 'has-error' :'' }}">
                            ...
                            <div class="col-sm-10">
                                <input type="number" step="any" value="{{ old('latitude',$hatchery->latitude) }}" name="latitude" class="form-control" id="latitude" placeholder="Enter latitude">
                                @error('latitude')
                                <span class="help-block">{{ $message }}</span>
                                @enderror
                            </div>
                        ...
                        <div class="form-group row {{ $errors->has('longitude') ? 'has-error' :'' }}">
                            ...
                            <div class="col-sm-10">
                                <input type="number" step="any" value="{{ old('longitude',$hatchery->longitude) }}" name="longitude" class="form-control" id="longitude" placeholder="Enter longitude">
                                @error('longitude')
                                <span class="help-block">{{ $message }}</span>
                                @enderror
                            </div>
                        ...
                        <div class="form-group row {{ $errors->has('address') ? 'has-error' :'' }}">
                            ...
                            <div class="col-sm-10">
                                <textarea  name="address" rows="2" class="form-control" id="address" placeholder="Enter address">{{ old('address',$hatchery->address) }}</textarea>
                                @error('address')
                                <span class="help-block">{{ $message }}</span>
                                @enderror
                            </div>
                        ...
                        <div class="form-group row {{ $errors->has('status') ? 'has-error' :'' }}">
                            ...
                            <div class="col-sm-10">
                                <div class="icheck-success d-inline pull-right">
                                    <input checked type="radio" id="active" name="status" value="1" {{ old('status',$hatchery->status) == '1' ? 'checked' : '' }}>
                                    <label for="active">
                                        Active
                                    </label>
                                </div>
                                <div class="icheck-danger d-inline pull-right">
                                    <input type="radio" id="inactive" name="status" value="0" {{ old('status',$hatchery->status) == '0' ? 'checked' : '' }}>
                                    <label for="inactive">
                                        Inactive
                                    </label>
                                </div>
                    ...
                    <div class="card-footer">
                        ...
                        <a href="{{ route('hatchery.index') }}" class="btn btn-danger bg-gradient-danger btn-sm float-right">Cancel</a>
                    </div>
                    <!-- /.card-footer -->
                </form>
            </div>
    ...
    <script>
        $(function (){
            $(document).ready(function () {
                const oldThanaId = "{{ old('thana',$hatchery->thana_id) }}";
                const oldDistrictId = "{{ old('district',$hatchery->district_id) }}";
