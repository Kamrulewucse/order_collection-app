    ...
    <div class="row">
        ...
        <div class="col-md-12">
            ...
            <div class="card card-default">
                ...
                <!-- /.card-header -->
                <!-- form start -->
                <form enctype="multipart/form-data" action="{{ route('campaign.store') }}" class="form-horizontal" method="post">
                    @csrf
                    <div class="card-body">
                        ...
                        <div class="form-group row {{ $errors->has('campaign_type') ? 'has-error' :'' }}">
                            ...
                            <div class="col-sm-10">
                                <select name="campaign_type" class="form-control select2" id="campaign_type">
                                    ...
                                    <option value="1" {{ old('campaign_type') == '1' ? 'selected' : '' }}>Online</option>
                                    <option value="2" {{ old('campaign_type') == '2' ? 'selected' : '' }}>Offline</option>
                                </select>
                                @error('campaign_type')
                                <span class="help-block">{{ $message }}</span>
                                @enderror
                        ...
                        <div class="form-group row {{ $errors->has('campaign_cost') ? 'has-error' :'' }}">
                            ...
                            <div class="col-sm-10">
                                <input type="text" value="{{ old('campaign_cost') }}" name="campaign_cost" class="form-control" id="campaign_cost" placeholder="0.00">
                                @error('campaign_cost')
                                <span class="help-block">{{ $message }}</span>
                                @enderror
                            </div>
                        ...
                        <div class="form-group row {{ $errors->has('details') ? 'has-error' :'' }}">
                            ...
                            <div class="col-sm-10">
                                <textarea type="text" name="details" class="form-control" id="details" placeholder="Enter details">{{ old('details') }}</textarea>
                                @error('details')
                                <span class="help-block">{{ $message }}</span>
                                @enderror
                            </div>
