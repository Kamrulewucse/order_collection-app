...
class SROrderController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            return redirect()->route('sr-sales.create', ['type' => $request->type, 'company' => $request->company])->withInput()->with('error', $e->getMessage());
        }
    }
    ...
    public function dayClosePost(DistributionOrder $distributionOrder, Request $request)
    {
        ...
        try {
           ...
           if (round($distributionOrder->total, 2) < round($customerBillAmount, 2)) {
            ...
            return redirect()
                ->route('sr-sales.day_close', ['distributionOrder' => $distributionOrder->id, 'type' => $distributionOrder->type])
    ...
    public function customerDamageProductEntryPost(DistributionOrder $distributionOrder, Request $request)
    {
        ...
        try {
            ...
            DB::commit();
            return redirect()->route('sr-sales.details', ['distributionOrder' => $distributionOrder->id, 'type' => $distributionOrder->type])
                ->with('message', 'Customer damage product updated successfully');
        } catch (\Exception $exception) {
            DB::rollBack();
            return redirect()->route('sr-sales.customer_damage_product_entry', ['distributionOrder' => $distributionOrder->id, 'type' => $distributionOrder->type])->withInput()
                ->with('error', $exception->getMessage());
        }
    }
    ...
    public function payment(Request $request)
    {
        ...
        try {
            ...
            return response()->json([
                'status' => true,
                'redirect_url' => route('sr-sales.details', ['distributionOrder' => $distributionOrder->id, 'type' => $request->type]),
